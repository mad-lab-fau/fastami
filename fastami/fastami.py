...
from sklearn.metrics.cluster import contingency_matrix, mutual_info_score, entropy, adjusted_mutual_info_score
from fastami.utils import WalkerRandomSampling


def adjusted_mutual_info_mc(labels_true: Sequence[int], labels_pred: Sequence[int], seed: Union[None, _ArrayLikeInt_co, SeedSequence, BitGenerator, Generator] = None, accuracy_goal: float = 0.01, min_samples: int = 10_000) -> Tuple[float, float]:
    ...
    if len(labels_true) != len(labels_pred):
        raise ValueError(
            "labels_true and labels_pred must have the same length.")
    ...
    while ami_err > accuracy_goal:
        ...
        ami_std = normalization*std(emi_arr, ddof=1) * abs(ami) * abs(
